...
export default function AuthModal({ isOpen, onClose, initialMode = 'signin', customMessage }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        {/* Custom message (e.g. "Build from scratch" context) */}
        {customMessage && (
          <div className="mx-6 mt-4 rounded-xl p-4 flex items-start gap-3"
            style={{ background: 'linear-gradient(135deg,#fdf4ff,#eff6ff)', border: '1px solid #e9d5ff' }}>
            <Sparkles size={16} style={{ color: '#d946ef', flexShrink: 0, marginTop: 2 }} />
            <p className="text-sm font-medium text-purple-800">{customMessage}</p>
          </div>
        )}
