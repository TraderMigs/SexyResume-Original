...
function parseResumeText(text: string): Partial<Resume> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentSection === 'personal') {
      // Extract name - must be 2-4 words, no special chars except hyphens/apostrophes, no job title keywords
      if (!personalInfo.fullName && i < 5 && line.length > 2 && line.length < 60 && !line.includes('@') && !line.includes('http')) {
        ...
        const hasSlashes = line.includes('/');
        const wordCount = line.split(/\s+/).length;
        
        // Name should be 2-4 words, no pipes/slashes, no job title keywords
        if (wordCount >= 2 && wordCount <= 4 && !hasPipes && !hasSlashes && !jobTitleKeywords.test(line)) {
          // Additional check: should only contain letters, spaces, hyphens, apostrophes
